...
class Request():
    ...
    def __init__(self, use_scraper=False) -> None:
        ...
        if not use_scraper:
            ...
            self.scraper.impersonate = "chrome110"
...
def urlretrieve(url, filename=None, reporthook=None, headers=None):
    ...
    fp = None
    try:
        r = requests.get(url, headers=headers, proxies=read_proxy(),
                         stream=True, timeout=timeout)
        r.raise_for_status()
        header = r.headers
        with open(filename, 'wb+') as fp_inner:
            fp = fp_inner
            bs = 1024
            size = -1
            blocknum = 0
        ...
        if fp and os.path.exists(filename):
            fp.close()
            os.remove(filename)
        raise
    finally:
