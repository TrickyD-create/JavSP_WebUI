...
def translate(texts, engine: Union[
    ...
    if engine.name == 'baidu':
        ...
        try:
            result = openai_translate(texts, engine.url, engine.api_key, engine.model, getattr(engine, 'system_prompt', None))
            if 'error_code' not in result:
                rtn = {'trans': result}
            else:
                err_msg = "{}: {}: {}".format(engine, result['error_code'], result['error_msg'])
...
def openai_translate(texts, url: Url, api_key: str, model: str, system_prompt: str = None, to="zh_CN"):
    ...
    if system_prompt:
        ...
        prompt = f"Translate the following Japanese paragraph into {to}, while leaving non-Japanese text, names, or text that does not look like Japanese untranslated. Reply with the translated text only, do not add any text that is not in the original content."
    data = {
         "messages": [
           {
             "role": "system",
