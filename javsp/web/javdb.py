...
_tried_config_cookie = False


def get_html_wrapper(url):
    """包装外发的request请求并负责转换为可xpath的html，同时处理Cookies无效等问题"""
    global request, cookies_pool, _tried_config_cookie
    ...
    if r.status_code == 200:
        ...
        if r.history and '/login' in r.url:
            ...
            if len(cookies_pool) == 0:
                if not _tried_config_cookie:
                    javdb_cookie = Cfg().network.javdb_cookie
                    if javdb_cookie:
                        cookies_dict = {}
                        for part in javdb_cookie.split(';'):
                            part = part.strip()
                            if '=' in part:
                                k, v = part.split('=', 1)
                                cookies_dict[k.strip()] = v.strip()
                        if '_jdb_session' in cookies_dict:
                            cookies_pool.append({
                                'profile': 'config.yml',
                                'site': 'javdb.com',
                                'cookies': cookies_dict,
                            })
                            _tried_config_cookie = True
                            logger.debug('从配置文件读取了 JavDB 的 Cookies (含 _jdb_session)')
                        else:
                            logger.warning('config.yml 中的 javdb_cookie 缺少 _jdb_session，已忽略')
            if len(cookies_pool) > 0:
