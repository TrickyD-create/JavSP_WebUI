...
def generate_tmdbid(name: str) -> str:
    """根据演员名字生成8位数的tmdbid"""
    # 使用SHA-1哈希函数生成哈希值
    hash_obj = hashlib.sha1(name.encode('utf-8'))
    # 将哈希值转换为十进制数值
    hash_int = int(hash_obj.hexdigest(), 16)
    # 取前8位数字作为tmdbid
    tmdbid = str(hash_int)[:8]
    # 确保tmdbid是8位数，如果不足则在前面补零
    tmdbid = tmdbid.zfill(8)
    return tmdbid
...
def write_nfo(info: MovieInfo, nfo_file):
    ...
    # NFO 只有 studio；优先使用制作商，缺失时回退到发行商。
    studio = info.producer or info.publisher
    if studio:
        nfo.append(E.studio(studio))
    ...
    if info.actress:
        for i in info.actress:
            if (info.actress_pics) and (i in info.actress_pics):
                if Cfg().summarizer.nfo.include_actor_tmdbid:
                    tmdbid = generate_tmdbid(i)
                    nfo.append(E.actor(E.name(i), E.thumb(info.actress_pics[i]), E.tmdbid(tmdbid)))
                else:
                    nfo.append(E.actor(E.name(i), E.thumb(info.actress_pics[i])))
            else:
                if Cfg().summarizer.nfo.include_actor_tmdbid:
                    tmdbid = generate_tmdbid(i)
                    nfo.append(E.actor(E.name(i), E.tmdbid(tmdbid)))
                else:
